...
import { centsToDollars, formatMoneyCompact } from "../../utils/money";

/**
 * Single source of truth for the merchant profile look & feel. Both the
 * signed-in screen (MerchantProfileScreen) and the public/guest screen
 * (guest/GuestMerchantProfileScreen) render this scaffold so the two can
 * never visually drift — they only differ in what the bottom CTA bar does.
 */
...
const ESTIMATED_CTA_BAR_HEIGHT = 118;
...
export const merchantDisplayName = (merchant?: Merchant | null) =>
  merchant ? merchant.store_name || merchant.name : "Comercio";

export const categorySubtitle = (merchant?: Merchant | null) => {
  const labels =
    merchant?.categories
...
export const resolveAmountPresets = (merchant?: Merchant | null) => {
...
/** Preset/selection amounts are whole dollars — rendered without cents.
 *  Anything actually charged (quote, total, balances) keeps formatMoney. */
export const amountLabel = (amountCents: number | null) =>
  formatMoneyCompact(centsToDollars(amountCents) ?? 0, "USD");
...
type ScaffoldProps = {
  ...
  onOtherAmount: () => void;
  /** Sticky bottom bar contents: primary CTA (+ optional secondary action). */
  children: React.ReactNode;
};

export const MerchantProfileScaffold: React.FC<ScaffoldProps> = ({
  merchant,
  amountPresets,
  selectedAmountCents,
  onBack,
  ...
  // The guest bar is taller than the signed-in one (it carries a second
  // action), so the scroll padding is measured instead of hardcoded.
  ...
  const scrollBottomPadding =
    (ctaBarHeight || insets.bottom + ESTIMATED_CTA_BAR_HEIGHT) + theme.spacing(2);

  return (
    <Screen style={styles.screen} edges={["top", "left", "right"]}>
      <AppHeader onBack={onBack} showBackLabel={false} style={styles.header} />
      <ScrollView
        showsVerticalScrollIndicator={false}
        contentContainerStyle={[styles.content, { paddingBottom: scrollBottomPadding }]}
      >
        <View style={styles.heroBand}>
          <MerchantGiftCardHero merchantName={displayName} logoUrl={merchant.logo_url} />
        </View>

        <View style={styles.titleBlock}>
          <Text style={styles.storeName}>{displayName}</Text>
          <Text style={styles.categorySubtitle}>{categorySubtitle(merchant)}</Text>
          ...
          <View style={styles.amountRow}>
            {amountPresets.map((amountCents) => {
              ...
              return (
                <Pressable
                  key={amountCents}
                  onPress={() => onSelectAmount(amountCents)}
      ...
      <View
        style={[styles.ctaBar, { paddingBottom: insets.bottom + theme.spacing(1) }]}
        onLayout={(event) => setCtaBarHeight(event.nativeEvent.layout.height)}
      >
        {children}
      </View>
    </Screen>
...
export const MerchantProfileLoading: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  const insets = useSafeAreaInsets();
...
export const MerchantProfileError: React.FC<{ onBack: () => void }> = ({ onBack }) => (
